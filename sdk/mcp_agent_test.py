#!/usr/bin/env python3
"""Clean-room agent test: drive Xplorer to search BKK->LA flights using ONLY the
...
goes through xplorer_mcp.py over stdio JSON-RPC, which auto-discovers the running
browser via ~/.xplorer/gateway.json.
"""
import json
import re
import subprocess
...
MCP = ["python3", __file__.replace("mcp_agent_test.py", "xplorer_mcp.py")]


class MCP:
    def __init__(self):
        self.p = subprocess.Popen(
            ["python3", sys.argv[0].replace("mcp_agent_test.py", "xplorer_mcp.py")],
...
def main():
    m = MCP()
    print("connected to Xplorer via MCP (auto-discovered, no token handling)\n")

    m.tool("xplorer_navigate", url="https://www.google.com/travel/flights?hl=en&curr=USD")
    time.sleep(2)

    def fill(field, query):
        els = json.loads(m.tool("xplorer_observe"))
        ref = next(e["ref"] for e in els if field in e["name"])
        m.tool("xplorer_type", ref=ref, text=query)
        time.sleep(1.5)
        # pick the first suggestion that matches the query (observe -> click)
        key = query.split()[0].lower()
        opts = json.loads(m.tool("xplorer_observe", role="option", limit=30))
        hit = next((o for o in opts if key in o["name"].lower()), None)
        assert hit, f"no suggestion for {query}"
        m.tool("xplorer_click", ref=hit["ref"])
    ...
    def daycells():
        return [e for e in json.loads(m.tool("xplorer_observe", role="gridcell",
                                             limit=400)) if e["name"]]
    cells = daycells()
    if not cells:
        els = json.loads(m.tool("xplorer_observe"))
        dep = next(e["ref"] for e in els if e["name"] == "Departure")
        m.tool("xplorer_click", ref=dep); time.sleep(1.2)
        cells = daycells()
    assert cells, "calendar did not open"
    m.tool("xplorer_click", ref=cells[14]["ref"]); time.sleep(0.5)
    cells = daycells()  # re-observe; refs shift after first pick
    m.tool("xplorer_click", ref=cells[35]["ref"]); time.sleep(0.5)
    ...
    if done is not None:
        m.tool("xplorer_click", ref=done); time.sleep(2)
    els = json.loads(m.tool("xplorer_observe"))
    srch = next(e["ref"] for e in els if e["name"].strip() == "Search")
    m.tool("xplorer_click", ref=srch)
    print("  clicked Search")

    # wait + read results
    for _ in range(30):
        if "departing flights" in m.tool("xplorer_read_text").lower():
            break
        time.sleep(0.5)
    txt = m.tool("xplorer_read_text")
